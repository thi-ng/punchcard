...
(defn load-commits-fs
  [repo-path author]
  (let [flt (if author #(pos? (.indexOf % author)) identity)]
    (->> (sh "git" "log" "--pretty=format:%aI %ae" :dir repo-path)
         :out
         str/split-lines
         (filter flt)
         (map #(-> % (str/split #" ") first ->cal)))))
...
(defn punch-card
  [{:keys [author gradient out repo title]}]
  (let [commits  (sort (load-commits-fs repo author))
...
(defn -main
  [& {:strs [--repo --author --title --gradient --out]
